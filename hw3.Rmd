...
data_cv8 <- college_clean %>% 
    filter(Apps < 30000 && P.Undergrad < 20000) %>% # remove outliers
    vfold_cv(v = 8)
...
spline_rec <- college_rec %>%
    step_ns(Apps, deg_free = 5) %>%
    step_ns(Top25perc, deg_free = 4) %>% 
    step_ns(P.Undergrad, deg_free = 4) %>%
    step_ns(Outstate, deg_free = 6) %>%
    step_ns(Room.Board, deg_free = 4) %>%
    step_ns(Personal, deg_free = 4) %>%
    step_ns(perc.alumni, deg_free = 4)
