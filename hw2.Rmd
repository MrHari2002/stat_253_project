...
Comparison of the MAE of the models show that the `ols_mod` is the best, because it has the lowest average MAE after 10-fold cross-validation (so there is few to none overfitting). This makes sense, because normally, we would assume that all the different musical features can contribute to the changes that occur in music over years. In other words, all variables are important, which leads to OLS having a lower MAE than LASSO models, since LASSO penalizes the inclusion of these important variables. 
...
2. **Summarize investigations**
  ...
  OLS has been the best model so far. Lasso did not seem to produce much accuracy among the music features. Including the penalty terms for coefficient does not generate yet better result. 
  We want to see the trend of each musical features across the years. We want to further improve the predictive accuracy. 
  We have very low p values, so we could reject the null hypothesis.
  Within the predictions the standard deviation is 11.24 years
  We have mean  absolute error of 12.0784177 years meaning the average predition error is 12.0784177 from the actual value.
  Prediction power, or the r squared is 0.3701227 not high, but not too low. This means the predictors explained 37 .01227 % of the variations in the model. 
  Explain the coefficients, especially danceability, loudness, acousticness and valence.
  We found out that the dancebility has a coefficent of 31.79 that means when the dancibility equals one the year predictions would add 31.79 year. However no music's dancibility would be 1 but the higher the dancibility the more recent the music is.
  Loudness has a coefficient of 62.1159212, that means when the loudness increases by one the prediction would increase by 62 years.
  Acousticness has a coefficient of -14.6899 which means as the acoustiness increases by 1 the year prediction would decrease by 14.6899.
  Valence has a coefficient of -24.7499699 which means as the valence increases by 1 the year prediction would decrease by 24.74.
  Based on the finding above we could infer that recent music compared to music that are older are louder,more suitable for dancing, has fewer acoustic components and more negative.
...
- Over-generallizing music trend
- Diversity, dataset is limited to English songs. 
- Features data collected depend on a third-party API that doesn't clearly explain how the features are measured, especially valence. https://developer.spotify.com/documentation/web-api/reference/#/operations/get-several-audio-features No access to the underlying formulae and algorithm
- Cautions: the prediction power, rsq is only 0.37 -> there are definitely trends in musical features, but the accuracy we can get when using those musical features to predict on new data is unreliable and varies a lot.
